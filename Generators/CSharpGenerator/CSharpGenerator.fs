...
let rec gCollectionType ct =
    match ct with
    | List itemType ->
        sprintf "List<%s>" (gFIDLType itemType)
    | Map (keyType,valueType) ->
        sprintf "Dictionary<%s,%s>" (gPrimitiveType keyType) (gFIDLType valueType)
...
let gFieldTypeDecl fieldTypeDecl =
    Concat [snd fieldTypeDecl |> gFIDLType; " "; fst fieldTypeDecl]
...
let gRecordType (rt:Record) =
    ...
    Group [
        Concat ["class "; QualifiedIdentifier.name rt.Identifier; " "]
